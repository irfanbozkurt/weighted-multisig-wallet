...
Weighted MultiSig Wallet provides pre-configured UI interfaces for Wallet governance and payment calls, but the users can craft a custom calldata and propose an entirely custom call to any CA or EOA. Inputed 'to', 'value' and '\_calldata' will be **call{}()** ed by the wallet contract given enough signature weight. Explore further all these interfaces in the online demo.
