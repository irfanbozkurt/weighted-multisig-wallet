...
- If it's a success, the tx is passed to the `call(){}` function of the deployed MetaMultiSigWallet contract (this contract), thereby passing the `onlySelf` modifier for any possible calls to internal txs such as (`transferFrom()`,`removeSigner()`,`transferFunds()`,`updateSignaturesRequried()`).
