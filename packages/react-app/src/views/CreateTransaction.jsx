  ...
  useEffect(() => {
    const inputTimer = setTimeout(async () => {
      ...
      try {
        ...
        //   console.log("🔥🔥🔥🔥🔥🔥",amount)
        //     const calldata = readContracts[walletContractName].interface.encodeFunctionData("transferFunds",[to,parseEther("" + parseFloat(amount).toFixed(12))])
        //     setData(calldata);
        // }
        // decodedDataObject = readContracts ? await readContracts[walletContractName].interface.parseTransaction({ data }) : "";
        // console.log("decodedDataObject", decodedDataObject);
        // setCreateTxnEnabled(true);

        if (decodedDataObject.signature === "transferFrom(address,address,uint256)") {
          setMethodName("transferFrom");
        ...
        <div style={{ margin: 8 }}>
          ...
          <div style={{ margin: 8, padding: 8 }}>
            <Select value={methodName} disabled={selectDisabled} style={{ width: "100%" }} onChange={setMethodName}>
              ...
              <Option disabled={true} key="transferFrom">
                transferFrom()
              </Option>
            </Select>
          </div>
