...
export default function Streams({
  ...
  const walletBalance = useBalance(
    localProvider,
    readContracts ? readContracts[walletContractName].address : readContracts,
  );
  ...
  useEffect(() => {
    let getStreams = async () => {
      ...
      for (let s in openStreamEvents) {
        if (openStreamEvents[s].to && newStreams.indexOf(openStreamEvents[s].to) < 0) {
          ...
          try {
            let update = {};
            update.stream = await readContracts[walletContractName].streams(openStreamEvents[s].to);
            if (update.stream && update.stream.amount.gt(0)) {
              update.balance = await readContracts[walletContractName].streamBalance(openStreamEvents[s].to);
            }
    ...
    if (readContracts && readContracts[walletContractName]) {
      getStreams();
    }
            ...
            return (
              <List.Item key={"withdrawal_" + index++}>
                <div>
                  ...
                  <Balance balance={item.amount} dollarMultiplier={price} />
                </div>
                {item.reason}
              </List.Item>
            );
      ...
      <List
        ...
        renderItem={item => {
          ...
          if (streamInfo[item] && !streamInfo[item].balance) {
                    ...
                    <Button
                      ...
                      onClick={() => {
                        if (!streamReason) {
                          ...
                          tx(writeContracts[walletContractName].streamWithdraw(amountToWithdraw, streamReason));
                          setStreamReason("");
            ...
            withdrawButtonOrBalance = (
              <Balance balance={streamInfo[item] ? streamInfo[item].balance : 0} dollarMultiplier={price} />
            );
          }
        ...
        <div style={{ margin: 8, padding: 8 }}>
          <Button
            onClick={() => {
              ...
              let calldata;
              if (methodName == "openStream") {
                calldata = readContracts[walletContractName].interface.encodeFunctionData("openStream", [
                  streamToAddress,
                  parseEther("" + parseFloat(streamAmount).toFixed(12)),
                  streamFrequency,
                ]);
              } else {
                calldata = readContracts[walletContractName].interface.encodeFunctionData("closeStream", [
                  streamToAddress,
                ]);
              }
